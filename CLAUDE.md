...
**Un guet sans échéance n'expire jamais.** « Si Ormuz rouvre » n'a pas de date : `echeance`
vaut `null`, et l'expiration ne s'évalue tout simplement pas. Un tel guet reste `ouvert`
jusqu'à ce qu'il soit résolu ou clos délibérément en `sans-objet`. Corollaire à ne pas
manquer : les guets remontés comptent dans les trois. Sans quoi trois guets sans date
...
**ONS (Royaume-Uni) — branchée, cinq séries sur six.** L'ancien point de terminaison
(`api.ons.gov.uk/timeseries/{id}/dataset/{ds}/data`) a été retiré le 25/11/2024 — confirmé par
appel réel, réponse « This API has been decommissioned ». `lib/ons.ts` interroge désormais
l'API de contenu du site (`api.ons.gov.uk/v1/data?uri=<chemin de la page timeseries>`), qui sert
le même schéma JSON (`months`/`quarters`/`years`, `description.unit`) : seule l'URL a changé,
pas le format de réponse. Chaque série porte donc un `topic` en plus de `timeseriesId` et
`datasetId` (`config/ons-series.ts`). IPCH total et sous-jacent, chômage et salaires sont
vérifiés par appel réel. La croissance du PIB l'est aussi, mais sur un CDID différent de celui
d'abord retenu : `ABMI` s'est révélé être le niveau du PIB en volumes chaînés (en millions de
livres), pas un taux — la série active est `IHYQ`, la croissance trimestre sur trimestre
précédent ; Eurostat publie trimestre sur même trimestre de l'année précédente
(`namq_10_gdp`, `CLV_PCH_SM`), donc les deux bases ne se comparent pas chiffre à chiffre en mode
comparaison. Le solde budgétaire reste désactivé (`enabled: false` sur `uk-budget-balance`,
